...
class NewsController extends BackController {
	...
	protected function processForm(HTTPReqest $request) {
		$news = new News([
		...
		if($news->isValid()) {
			...
			$this->app->user()->setFlash($news->isNew() ? 'La a bien été ajoutée !' : 'La news a bien été modifiée !');
		}
		else 
	...
	public function executeUpdateComment(HTTPRequest $request) {
		...
		if($request->postExists('pseudo')) {
			...
			if($comment->isValid()) {
				...
				$this->app->httpResponse()->redirect('/news-'.$comment->postData('news').'.html');
			}
			else 
