...
class MemberController extends BackController {
	public function executeSignUp(HTTPRequest $request) {
		if($request->method() == 'POST') {
			...
			$namesValues = array('pass_repeat' => $request->postData('pass_repeat'));
		}
		else {
			$membre = new Member();
			$namesValues = array('pass_repeat' => null);
		}
			
		$formBuilder =  new MemberFormBuilder($membre, $namesValues);
		$formBuilder->build();
		$form = $formBuilder->form();
		$formHandler = new FormHandler($form, $this->managers->getManagerOf('Members'), $request);
		$user = $this->app->user();
		if($formHandler->processRegister()){
			$user->setFlash('Inscription effectuée avec succès');
	...
	public function executeDelete(HTTPRequest $request) {
		...
		if($request->method() == 'POST') {
			...
			if($formHandler->processDeleteMember()) {
				...
				$this->page->addVar('form', $form->createForm());
	}
	
	public function executeInviteFriend(HTTPRequest $request) {
		$user = $this->app->user();
		$this->managers->getManagerOf('Members')->inviteFriend($request->getData('id'), $user->memberId());
		$user->setFlash('Votre requête a bien été envoyée');
		$this->app->httpResponse()->redirect('.');
	}
	
	public function executeAddFriend(HTTPRequest $request) {
		$user = $this->app->user();
		$manager = $this->managers->getManagerOf('Members');
		$manager->addFriend($request->getData('id'), $user->memberId());
		$friend = $manager->getUnique($request->getData('id'));
		$user->setFlash($friend->pseudo() + ' a bien été ajouté à votre liste d\'amis');
		$this->app->httpResponse()->redirect('.');
	}
}